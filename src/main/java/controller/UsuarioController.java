...
public class UsuarioController implements IUsuarioController {
    ...
    @Override
    public String verDisponibilidad(String username) {

        DBConnection con = new DBConnection();
        String sql = "Select id,count(*) as disponible from alquiler where username = '"
        ...
        try {
            ...
            while (rs.next()) {
                ...
                disponibilidad.put(id, disponible);
            }

            devolverCuarto(username, disponibilidad);
    ...
    @Override
    public String devolverCuarto(String username, Map<Integer, Integer> disponibilidad) {

        DBConnection con = new DBConnection();

        try {
            for (Map.Entry<Integer, Integer> cuarto : disponibilidad.entrySet()) {
                int id = cuarto.getKey();
                int disponible = cuarto.getValue();

                String sql = "Update cuartos set disponibilidad = (Select disponibilidad + " + disponible +
                        " from cuartos where id = " + id + ") where id = " + id;
